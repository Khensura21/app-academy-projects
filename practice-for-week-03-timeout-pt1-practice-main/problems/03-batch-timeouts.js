...
function batchTimeouts(callbacks, delays) {
  // Your code here
  // loop through the call back array via forEach
  // we are going to pass the corresponding cb and delayed time at each idx to the async function setTimeout
  let timeOutObjs = callbacks.map((cb, idx) => {

    let cbDelay = delays[idx];

    return setTimeout(cb, cbDelay);
