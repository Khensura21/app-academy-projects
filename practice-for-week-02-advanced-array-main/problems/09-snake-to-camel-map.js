...
function snakeToCamel(str) {
    ...
    const capitlizeFirstChar = function (str) {

        let chars = str.toLowerCase().split('');
        let firstCharUpperCased = chars[0].toUpperCase();

        chars[0] = firstCharUpperCased;

        return chars.join('');

    }

    let upperCasedWords = words.map(capitlizeFirstChar);
