...
function dynamicIntervalCount(cb, delay, amount) {
  // Your code here
  //create a var to hold the Timeout object amount
  let interval =
    //call the set interval method, passing in an annonymous function, with the delay argument
    setInterval(function () {
      // invoke the callback
      cb();
      if (amount) {
        // then check to see if an amount argument is passed. if so decrement it,
        // decrement amount until it reach zero it until it reaches z
        amount--;
        if (amount === 0) {
          clearInterval(interval);
        }
      }
      //otherwise the cb will run forever...

    }, delay);

  //return setInterval object
  return interval;
}
